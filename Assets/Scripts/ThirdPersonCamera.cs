...
public class ThirdPersonCamera : MonoBehaviour
{
    ...
    private void Update()
    {
        Vector3 targetPositiion = orientation.position + (orientation.right * offset.x) + (orientation.up * offset.y) + (orientation.forward * offset.z);
        Quaternion targetRotation = Quaternion.LookRotation(orientation.forward);
        
        transform.position = Vector3.Lerp(transform.position, targetPositiion, smoothFactor * Time.deltaTime);
